...
`--only-new --apply` requires `--resumable` and `--source-page-key`. `date_change` must be selected as a native Oracle `TIMESTAMP`. The loader first freezes and materializes only the changed membership IDs, timestamps, and page-key tuples in its local SQLite checkpoint, then fetches complete rows by ID batches (at most 1,000 IDs per Oracle query). The source page key breaks timestamp ties. No loader-only columns are added to integration SQL, and there is no separate full-window count query.

The incremental path trusts KN globally to provide non-null, unique membership IDs and non-null, unique `(watermark, source-page-key...)` cursor tuples; it does not perform an expensive full-window duplicate/null scan. Each materialized key page and each ID payload batch is checked before its PostgreSQL transaction, so malformed or changed source data stops the uncommitted page. A duplicate that occurs only across different key pages remains a violated KN source contract and is not globally scanned.

For a KN composite primary key, keep the destination's synthetic concatenated `*_pk` as `--id-field`, but also select the raw KN key columns under normal aliases and pass those aliases to `--source-page-key` in the exact KN index order. Those extra aliases are local loader fields only: they are retained in SQLite and used for indexed payload lookups, never inserted into PostgreSQL. Direct-ID integrations keep using their `*_pk` alias for both options.
