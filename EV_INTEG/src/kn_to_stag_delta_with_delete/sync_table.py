#!/usr/bin/env python3
"""Canonical catalog-driven KN→staging CLI.

Thin sibling wrappers select one fixed table; this command accepts any active
catalogued target table as its required positional value.
"""
...
from integration_core.kn_staging import (
    KnStagingRun,
    freeze_upper_watermark,
    read_select,
    source_columns,
    target_columns,
    upsert_sql,
    validate_oracle_index,
    validate_selected_columns,
    validate_source_shape,
    validate_target_membership_index,
)
...
def parse_args(fixed_table: str | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("table", nargs="?" if fixed_table else None, default=fixed_table)
    ...
    mode.add_argument("--status", action="store_true", help="show checkpoint state without connecting to either database")
    ...
    parser.add_argument("--fresh", action="store_true", help="discard this table's full-sync checkpoint")
    ...
    if args.status:
        ...
        for enabled, option in (
            (args.resumable, "--resumable"), (args.fresh, "--fresh"),
            (args.auto_page_size, "--auto-page-size"),
            (args.initial_page_size is not None, "--initial-page-size"),
            (args.max_page_size is not None, "--max-page-size"),
            (args.constant_page_size is not None, "--constant-page-size"),
            (args.page_size is not None, "--page-size"),
            (args.confirm_staging_already_current, "--confirm-staging-already-current"),
        ):
            if enabled:
                irrelevant.append(option)
        if irrelevant:
            parser.error("--status cannot be combined with " + ", ".join(irrelevant))
    if args.only_new and not args.resumable and not args.status:
    ...
    if args.apply and not args.resumable:
        parser.error("KN-to-staging apply requires --resumable")
    if args.dry_run and (args.fresh or args.auto_page_size or args.initial_page_size is not None or args.max_page_size is not None or args.constant_page_size is not None):
        parser.error("dry-run does not accept checkpoint or page-size tuning options")
    args.page_size = args.page_size or 50_000
    args.spec = spec
    return args
...
def load_status_environment() -> None:
    """Load identity settings only; importing this path never imports drivers."""
    from dotenv import load_dotenv
    load_dotenv(HERE / ".env", override=False)
...
def replace_data_connections(run: Any, connect_oracle: Any, connect_staging: Any) -> None:
    """Acquire both replacements before atomically swapping live sessions."""
    new_source = new_destination = None
    try:
        new_source = connect_oracle()
        new_destination = connect_staging()
        new_destination.autocommit = True
    except BaseException:
        for connection in (new_source, new_destination):
            close = getattr(connection, "close", None)
            if callable(close):
                close()
        raise
    old_source, old_destination = run.source_connection, run.destination_connection
    run.source_connection, run.destination_connection = new_source, new_destination
    for connection in (old_source, old_destination):
        close = getattr(connection, "close", None)
        if callable(close):
            close()
...
def print_progress(event: dict[str, Any]) -> None:
    """One CLI formatter for durable preflight and payload page progress."""
    elapsed = timedelta(seconds=round(event["elapsed_seconds"]))
    eta = event.get("eta_seconds")
    eta_text = str(timedelta(seconds=round(eta))) if eta is not None else "unknown"
    print(
        f"{event['phase']} page {event['page']} "
        f"({event['page_rows']} rows; total {event['rows']}; "
        f"{event['rate']:.0f} rows/s; elapsed {elapsed}; ETA {eta_text})",
        flush=True,
    )
...
def preview(kn: Any, staging: Any, query: str, spec: Any, only_new: bool) -> None:
    """Validate the exact apply contract using read-only database sessions."""
    source = source_columns(kn, query)
    validate_selected_columns(source, spec, require_change=True)
    ...
    destination = target_columns(staging, spec)
    validate_target_membership_index(staging, spec)
...
def initialize_only_new_watermark(
    ...
    The acknowledgement belongs at the CLI boundary. This function validates
    the declared source shape/index, reads the native upper tuple under writer
    locks, and writes only an *incomplete* window checkpoint—never a staging
    payload row. The transfer validates the exact frozen result while
    materializing it before this can become a completed lower cursor.
    """
...
def main(fixed_table: str | None = None) -> int:
    ...
    if args.status:
        # The database name is part of the checkpoint fingerprint. Status must
        # see the same dotenv-configured identity as apply while remaining
        # completely free of Oracle/PostgreSQL driver imports or connections.
        load_status_environment()
    ...
    with oracledb.connect(**oracle_settings(oracledb)) as kn:
        if args.dry_run or (not args.apply and not args.initialize_only_new_watermark):
            with psycopg.connect(**pg_settings()) as staging:
                staging.read_only = True
                preview(kn, staging, query, spec, args.only_new)
            return 0
        # Apply requires checked-in Oracle index metadata for the declared
        # native page tuple; no index name is guessed at runtime.
        if spec.oracle_owner is None or spec.oracle_index is None:
            raise RuntimeError(f"{spec.target_table} is not runnable until its verified Oracle index metadata is catalogued")
        ...
        with psycopg.connect(**pg_settings()) as staging, InterruptController(lambda message: print(message, file=sys.stderr, flush=True)) as interrupts:
            ...
            run = KnStagingRun(
                spec, kn, staging, query, args.page_size, path, run_id, context,
...
def cli(fixed_table: str | None = None) -> int:
    """Process boundary shared by the canonical command and thin wrappers."""
    try:
        return main(fixed_table)
