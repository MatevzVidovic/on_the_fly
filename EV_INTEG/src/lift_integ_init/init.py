...
from integration_core import WriterRunContext
...
def load_environment() -> None:
    ...
    # Each active command owns one explicit environment file.
...
def writer_context(psycopg: Any, spec: Any, *, apply: bool) -> Any:
    """Serialize an applied watermark with writers of its staging table."""
    if not apply:
        return nullcontext()
    database = os.environ.get("STAG_DATABASE", "fmp_data_gurs")
    return WriterRunContext(
        STATE_ROOT / spec.target_table / "lift-init.lock",
        lambda: psycopg.connect(**pg_settings(database)),
        database, SCHEMA, spec.target_table,
    )
...
def guarded_metadata_update(
    writer: Any, metadata: Any, integration_id: Any, maximum: Any,
    previous: Any, proposed_start: datetime | None, previous_start: Any | None,
) -> None:
    """Mutate metadata while the staging database's session lock stays held.

    Advisory locks are database-local. The writer lock is in the staging data
    database, so handing it to a transaction in ``fmp`` would briefly release
    the real staging mutex and acquire an unrelated lock instead.
    """
    metadata.commit()
    writer.ensure_held()
    with metadata.transaction():
        ...
        with metadata.cursor() as cursor:
            apply_metadata_update(
                cursor, integration_id, maximum, previous,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # Apply freezes MAX(date_change) and updates LIFT metadata while the
        # same table lock as KN→staging is held. Dry-run is best-effort.
        with writer_context(psycopg, spec, apply=args.apply) as writer:
            if args.apply:
                writer.ensure_held()
            with psycopg.connect(**pg_settings(data_database)) as data:
            ...
            with psycopg.connect(**pg_settings("fmp")) as metadata:
                ...
                if not args.apply:
                    return 0
                # End the read/preview transaction before the guarded write.
                # The transaction-scoped advisory lock then closes the tiny
                # race between checking the dedicated writer session and the
                # metadata UPDATE itself.
                guarded_metadata_update(
                    writer, metadata, integration_id, maximum, previous,
                    proposed_start, previous_start[0] if previous_start else None,
                )
                writer.ensure_held()
