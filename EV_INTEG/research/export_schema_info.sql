...
from (
    ...
    select '02_TABLE', t.table_name, null, null, null, null, null, null, null,
           to_char(t.num_rows), to_char(t.avg_row_len), to_char(t.last_analyzed, 'YYYY-MM-DD HH24:MI:SS'), tc.comments
    ...
    select '04_CONSTRAINT', ac.table_name, ac.constraint_name, acc.position, acc.column_name,
           case ac.constraint_type when 'P' then 'PRIMARY KEY' when 'U' then 'UNIQUE'
                when 'R' then 'FOREIGN KEY' when 'C' then 'CHECK OR NOT NULL' else ac.constraint_type end,
           case ac.constraint_type when 'P' then 'PRIMARY KEY' when 'U' then 'UNIQUE'
                when 'R' then 'FOREIGN KEY' when 'C' then 'CHECK' else ac.constraint_type end,
           rc.table_name, rcc.column_name, ac.status, ac.validated, ac.delete_rule,
           case when ac.constraint_type = 'R' then 'REFERENCES ' || rc.owner || '.' || rc.table_name || '(' || rcc.column_name || ')'
                when ac.constraint_type = 'C' then 'CHECK CONDITION OMITTED FROM THIS EXPORT BECAUSE ORACLE STORES IT AS LONG'
                else 'DEFERRABLE=' || ac.deferrable || '; DEFERRED=' || ac.deferred end
    ...
    select '05_INDEX', i.table_name, i.index_name, ic.column_position, ic.column_name,
           ...
           'STATUS=' || i.status || '; PARTITIONED=' || i.partitioned || '; COMPRESSION=' || nvl(i.compression, 'DISABLED')
      from all_indexes i
    ...
    union all
    select '06_LOB', l.table_name, l.segment_name, null, l.column_name, 'LOB SEGMENT',
           null, null, null, l.securefile, l.in_row, l.compression,
           'LOB_INDEX=' || l.index_name || '; DEDUPLICATION=' || l.deduplication || '; CACHE=' || l.cache || '; LOGGING=' || l.logging
