...
def test_markdown_includes_all_required_columns():
    output = check.markdown([{"table": "ev_x_h", "metadata": "PASS", "sync_start": "PASS", "delta_config": "PASS", "data": "CACHED", "highwater": "PASS", "lift_now": "PASS", "delta": "PASS", "result": "PASS", "detail": "ok"}], "staging")
    assert "Zero newer rows" in output
    assert "LIFT now" in output
    assert "Last sync start" in output
    assert "Delta config" in output
...
def test_lift_full_sync_counts_all_rows_without_date_filter():
    ...
    assert not result[2] and result[4] == 2
...
def test_equal_timestamp_is_a_lift_candidate_due_to_inclusive_lower_bound():
    ...
    assert not result[2] and result[4] == 1
...
def _install_check_one_mocks(monkeypatch, *, titles=("EV H Example",), connection="KN ORACLE", output=None):
    ...
    monkeypatch.setattr(check, "highwater_and_delta", lambda *_args: calls.append("water") or (True, "ok", True, "ok", 0, True, "ok", 0))
    return calls
...
def test_null_last_sync_start_is_a_metadata_failure_but_keeps_independent_checks_running(monkeypatch):
    ...
    result = check.check_one(None, None, None, _comparison_spec(), "staging", {"entries": {}}, False, 10, "manifest")
    assert result["sync_start"] == "FAIL" and result["metadata"] == "FAIL"
...
def test_full_sync_is_a_metadata_and_delta_configuration_failure(monkeypatch):
    ...
    result = check.check_one(None, None, None, _comparison_spec(), "staging", {"entries": {}}, False, 10, "manifest")
    assert result["metadata"] == "FAIL"
    assert result["delta_config"] == "FAIL"
